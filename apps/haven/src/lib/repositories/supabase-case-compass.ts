...
function mapCommunityPosts(rows: CommunityPostRow[], commentRows: CommunityPostCommentRow[]): CommunityPost[] {
  return rows.map((row) => ({
...
async function loadCommunitySnapshot(
  ...
  const [
    ...
    context.supabase.from("community_post_comments").select("*")
  ]);

  const safeCommentRows = isMissingCommunityPostCommentsTable(commentRowsError) ? [] : commentRows;

  if (!communitySpaceRows || !membershipRows || !postRows || !safeCommentRows) {
    return {
      cohorts: context.snapshot.cohorts,
      warRoom: context.snapshot.warRoom
    };
  ...
  const hasRealCommunityData =
    communitySpaceRows.length > 0 ||
    membershipRows.length > 0 ||
    postRows.length > 0 ||
    safeCommentRows.length > 0;

  return {
    cohorts: hasRealCommunityData ? cohorts : context.snapshot.cohorts,
    warRoom: hasRealCommunityData ? warRoom : context.snapshot.warRoom
  };
}
