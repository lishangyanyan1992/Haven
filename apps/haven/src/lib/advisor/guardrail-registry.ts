...
const ENTRIES: GuardrailEntry[] = [
  ...
  },
  {
    ...
    lastReviewedBy: null,
    lastReviewedAt: null,
    text: "H-1B layoff/transfer: separate ability to remain in the U.S. from ability to work. State each of these once, where it is relevant, and never again in a summary or reminder section: 'Do not work without authorization' and 'LCA preparation alone does not preserve status.' Do not suggest an unpaid role, volunteer role, or temporary position as a way to preserve H-1B status. Do not use last paycheck as the grace-period trigger unless the source and facts support it. Mention that the grace period is up to 60 days or until I-94/petition validity ends, whichever is shorter. For a new employer, preparation, LCA work, or documents sitting with the employer are not the same as a properly filed nonfrivolous H-1B petition. If the user gives dates, calculate the rough timeline and say what must be filed before day 60. Tell the user to confirm the exact deadline and filing strategy with immigration counsel immediately."
  },
  {
